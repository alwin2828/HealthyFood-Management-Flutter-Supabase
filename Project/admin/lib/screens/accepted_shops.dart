...
class AcceptedShops extends StatefulWidget {
  const AcceptedShops({super.key});

  @override
  State<AcceptedShops> createState() => _AcceptedShopsState();
}

class _AcceptedShopsState extends State<AcceptedShops> {
  ...
  Future<void> fetchShops() async {
    try {
      final response = await supabase.from('tbl_shop').select().eq('shop_vstatus', '1');
      print('Accepted shops response: $response');
      setState(() {
        acceptedShops = response;
        isLoading = false;
      });
    } catch (e) {
      setState(() {
